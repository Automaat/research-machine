...
**Research Brief usage:** Check "Existing Patterns" — if the codebase already has a shared solution, the PR creating a parallel one is a red flag. Use "Architecture Context" for ADR violations. "Callers & Consumers" reveals how many teams/modules depend on changed interfaces.
...
**Research Brief usage:** "Callers & Consumers" is critical here — exact caller counts determine whether an API change is safe or needs expand-and-contract migration. "Existing Patterns" reveals if API versioning conventions exist.
...
**Research Brief usage:** "Callers & Consumers" quantifies blast radius with real numbers. "Git History" reveals area volatility — high-churn areas with failure-prone changes deserve extra scrutiny. "Related Tests" shows whether failure paths are actually tested.
...
**Research Brief usage:** "Callers & Consumers" traces data flow — identify where untrusted input enters changed functions. "Existing Patterns" shows whether similar code validates inputs consistently. "Architecture Context" surfaces security-related ADRs.
...
**Research Brief usage:** "Callers & Consumers" directly answers "which teams consume affected APIs" with real file paths and counts instead of speculation. "Existing Patterns" reveals competing implementations that signal missing ADRs.

...
description: Analyze the current repo for technical debt, research version-specific best practices, and create an umbrella GitHub tracker issue with one subissue per finding. Use when the user asks to audit, find, or track technical debt in a code repository.
metadata:
  short-description: Audit a repo for technical debt and file a tracked umbrella issue
---
...
- If no remote, save as local markdown file instead: `${XDG_DATA_HOME:-$HOME/.local/share}/technical-debt-manager/debt-report-{date}.md` (skip rest of Phase 5)
...
- **No GitHub remote:** Save report as `${XDG_DATA_HOME:-$HOME/.local/share}/technical-debt-manager/debt-report-{date}.md`
